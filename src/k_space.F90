module Class_k_space
   ...
   type k_space
      ...
      integer(int32)              :: DOS_num_k_pts !> number of kpts per dim
      integer(int32)              :: berry_num_k_pts !> number of ks per dim in berry calc
      integer(int32)              :: ACA_num_k_pts
      integer(int32)              :: num_DOS_pts!> number of points on E grid
      integer(int32)              :: num_k_pts !> number of k_pts per segment
      integer(int32)              :: num_plot_pts !> points per dimension
      integer(int32)              :: nProcs !> number of MPI Processes
      integer(int32)              :: me !> MPI rank
      integer(int32)              :: berry_iter !> number of grid refinements
      integer(int32)              :: kpts_per_step !> new kpts per step and Proc
   ...
   contains
      ...
      procedure :: integrate_hall_surf    => integrate_hall_surf
...
contains
   ...
   subroutine calc_pdos(self, E, PDOS)
      ...
      integer(int32)     :: first, last, ierr
      integer(int32)     :: k_idx, E_idx, j, m, N, info,istat(1) = 0
      integer(int32)     :: lwork, liwork, lrwork, percentage
...
!$OMP             parallel do private(m,j,lor) default(shared)
   ...
   subroutine calc_and_print_dos(self)
      ...
      real(dp)                           :: dE
      integer(int32)                           :: i, num_up, istat(8) = 0
   ...
   subroutine setup_k_grid(self)
      ...
      integer(int32)              :: sz_x, sz_y, i, j, istat(1) = 0
   ...
   subroutine setup_inte_grid_para(self, n_k, padding)
      ...
      real(dp)               :: k1(3), k2(3)
      integer(int32)               :: i, j, cnt, istat(1) = 0
   ...
   subroutine setup_inte_grid_para_spiral(self, n_k, padding)
      ...
      real(dp)               :: k1(3), k2(3)
      integer(int32)               :: i, j, cnt, NAPD, rat, n1, n2, scale, istat(1) = 0
      logical, optional     :: padding
   ...
   subroutine setup_inte_grid_hex(self, n_k)
      ...
      real(dp)                :: den, l, a
      integer(int32)                :: cnt_k, start, halt, my_n, i, istat(1) = 0
   ...
   subroutine setup_k_path_abs(self)
      implicit none
      class(k_space)        :: self
      integer(int32)               :: n_pts, n_sec, start, halt, i, istat(1) = 0
   ...
   subroutine setup_k_path_rel(self)
      ...
      integer(int32)    ::  n_pts, n_sec,i,j, start, halt, cnt, istat(1) = 0
   ...
   subroutine calc_berry_quantities(self,pert_log)
      ...
      integer(int32)   , allocatable  :: kidx_all(:), kidx_new(:)
      integer(int32)     :: N_k, num_up, iter, n_ferm,nProcs
      integer(int32)     :: all_err(19), info
   ...
   subroutine calc_new_kidx(self, kidx_new)
      implicit none
      class(k_space)               :: self
      integer(int32)   , allocatable      :: kidx_new(:)
      integer(int32)                      :: cnt, N_k, k_idx
   ...
   subroutine calc_new_berry_points(self, eig_val_new, omega_z_new, omega_surf_new, omega_sea_new, Q_L_new, Q_IC_new, pert_log)
      ...
      implicit none
      class(k_space)            :: self
      integer(int32)                   :: N_k, cnt, k_idx, num_up, n_ferm,pert_idx
      integer(int32)                   :: first, last, err(6), me, ierr
      real(dp)                   :: tmp
      real(dp)                   :: k(3)
      ...
      if(pert_log) then
         ...
         do k_idx = first, last
            k = self%new_k_pts(:,k_idx)
            call self%ham%calc_eig_and_velo(k, eig_val_new(:,cnt), del_kx, del_ky,0)
            if(self%calc_hall ) then!.or. self%calc_hall_diag
               call self%ham%calc_berry_z(omega_z_new(:,cnt),&
                                       eig_val_new(:,cnt), del_kx, del_ky)
            endif
            if(self%calc_hall_diag) then
   ...
   function process_hall(self, var, var_old, iter, varall) result(cancel)
      ...
      integer(int32)                        :: send_count, ierr, N
      integer(int32)   , allocatable        :: num_elems(:), offsets(:)
   ...
   subroutine integrate_hall_sea(self, kidx_all, omega_z_all, hall)
      ...
      implicit none
      class(k_space)          :: self
      integer(int32)   , intent(in)  :: kidx_all(:)
      real(dp), intent(in)     :: omega_z_all(:,:)
      real(dp), allocatable    :: hall(:)
      integer(int32)                 :: loc_idx, n_hall, k_idx
      integer(int32)                 :: ierr(2), all_err(1)
      all_err = 0
   ...
   subroutine integrate_hall(self, kidx_all, omega_z_all, eig_val_all, hall)
      ...
      real(dp), allocatable    :: hall(:)
      real(dp)                 :: ferm
      integer(int32)                 :: loc_idx, n, n_hall, k_idx
      integer(int32)                 :: ierr(2), all_err(1)
   ...
   subroutine integrate_orbmag(self, Q_kidx_all, Q_L_all, Q_IC_all, orb_mag, orbmag_L, orbmag_IC)
      ...
      implicit none
      class(k_space)          :: self
      integer(int32)   , intent(in)  :: Q_kidx_all(:)
      real(dp), intent(in)     :: Q_L_all(:, :), Q_IC_all(:,:)
      real(dp), allocatable    :: orb_mag(:), orbmag_L(:), orbmag_IC(:)
      integer(int32)                 :: n_ferm, loc_idx, k_idx, E_f_size
      integer(int32)                 :: ierr(4)
   ...
   subroutine set_hall_weights(self, omega_z_all, kidx_all)
      ...
      integer(int32)   , intent(in) :: kidx_all(:)
      real(dp)                :: omega_z_all(:,:)
      integer(int32)                :: i, node, k_idx, loc_idx
      integer(int32)                :: n_elem
      integer(int32)                :: ierr(2), error(2) = [0,0]
   ...
   subroutine set_orbmag_weights(self, Q_all, Q_kidx_all)
      ...
      integer(int32)   , intent(in)    :: Q_kidx_all(:)
      integer(int32)                   :: i, node, k_idx, loc_idx
      integer(int32)                   :: ierr(2), error(2), n_elem
      character(len=300)        :: msg
...
!$OMP         parallel do private(n, m, Ef, f_nk, dE) collapse(2) default(shared)
   ...
   subroutine append_eigval(eig_val_all, eig_val_new)
      ...
      integer(int32)    :: vec_sz, num_k_all, num_k_new, i,j
      integer(int32)    ::ierr(6)
   ...
   subroutine append_quantity(quantity_all, quantity_new, reuse)
      ...
      integer(int32)                 :: vec_sz, num_k_old, num_k_new
   ...
   subroutine pad_k_points_init(self)
      implicit none
      class(k_space)                :: self
      integer(int32)     :: rest, i,j, cnt, n_kpts, n_elem, per_proc, istat(1) = 0
      integer(int32)   , allocatable :: sort(:)
   ...
   subroutine add_kpts_iter(self, n_new, new_ks)
      ...
      integer(int32)                 :: n_elem, i, cnt, area_cnt, weight_cnt, num_kpts
      integer(int32)   , allocatable :: sort_weight(:), sort_area(:)
   ...
   subroutine append_kpts(self)
      ...
      class(k_space)         :: self
      real(dp), allocatable   :: tmp(:,:)
      integer(int32)                :: old_sz, new_sz, i,j
      integer(int32)                :: ierr(2)
   ...
   subroutine calc_ACA(self)
      ...
      integer(int32)                 :: lwork, lrwork, liwork
...
!$OMP         end parallel do
   ...
   function calc_local_l(self, eig_vec, eig_val, n_ferm) result(loc_l)
      ...
      integer(int32)                        :: i, s, n_stat, v_u, v_d
